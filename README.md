...
- Merge upstream/master (aMule 3.0.0) into broadband branch:
  - Fix bug when maxUploadDataRateClient = 0
  - Feature: Async disk I/O for uploads (eMule's CUploadDiskIOThread + adaptive chunks)
  - Feature: Async disk I/O for downloads (eMule's CPartFileWriteThread)
  - Feature: Async per-part hash verification (CPartFileHashThread)
  - Feature: Speed limit types widened from uint16 to uint32 (supports rates > 64 KB/s per field)
  - Feature: MaxUpload=0 now means literal unlimited
  - Feature: MaxDownload enforced as a literal byte/sec global cap
  - Feature: Shared files auto-rescan via wxFileSystemWatcher
  - Feature: WebUI Kad connect/disconnect + ED2K/Kad network-enable checkboxes
  - Improvement: PartFile reduced disk I/O (dirty flag, atomic-rename, 10-min heartbeat stats)
  - Improvement: Boost ASIO mandatory (removed wxWidgets-sockets fallback)
  - Improvement: Throttler signals on empty-to-non-empty control-queue transition
